...
from models.diffusion_module import DDPM_sampler
from models.diffusion_module.Unet import UNet
from .GMM import BayesCap_MLP
from .prob_setting_embeding.aggregation import aggre_module
from .prob_setting_embeding.slot_attention import MLP
...
class DEM(nn.Module):
    def __init__(self, config=None, load_vision_params=False, load_text_params=False,
                 ...
                 use_mlm_loss=False, config_text=None,device=None):
        super().__init__()
        ...
        if config['diffusion']:
            self.eps_model = UNet(image_channels = 1, n_channels = 16,
                                    ch_mults = (1, 2, 2, 4),
                                    is_attn = (False, False, True, True),
                                    n_blocks= 2)
            self.ddpm_sampler = DDPM_sampler.DenoiseDiffusion(eps_model=self.eps_model, n_steps=1000, device=device)
        if config['gmm']:
            self.text_prob_con = BayesCap_MLP(256, 256, 512, 5, 0.1)
            self.image_prob_con = BayesCap_MLP(256, 256, 512, 5, 0.1)
        
        if config['slot']:
            self.agm = aggre_module(config, device=device)

        if config['soft_margin']:
            self.soft_mlp1 = MLP(config['embed_dim'], 512, config['embed_dim'])
            self.soft_mlp2 = MLP(config['embed_dim'], 512, config['embed_dim'])
            self.soft_fc = nn.Linear(config['embed_dim'], config['embed_dim'])
    ...
    def triplet_margin_loss(self, anchor, positive, negative, margin):
        # pos_distance = torch.sum((anchor - positive) ** 2, dim=-1)
        # neg_distance = torch.sum((anchor - negative) ** 2, dim=-1)
        pos_distance = torch.einsum('be,be->b', [anchor, positive])
        neg_distance = torch.einsum('be,be->b', [anchor, negative])
        # loss = torch.clamp(pos_distance - neg_distance + margin, min=0.0)
        loss = torch.clamp(neg_distance - pos_distance + margin, min=0.0)

        return torch.mean(loss)
    def get_soft_match_loss(self, dist_text, margin, image_feats, text_feats):
        similarity = text_feats @ image_feats.t()
        mask = torch.eye(image_feats.shape[0], dtype=torch.bool)
        similarity[mask] = float('-inf')
        # print(similarity)
        img_hard_negatives = torch.argmax(similarity, dim=1)
        txt_hard_negatives = torch.argmax(similarity.t(), dim=1)
        margin = (torch.pow(margin, dist_text) - 1) / (margin - 1)
        loss = self.triplet_margin_loss(text_feats, 
                                        image_feats, 
                                        image_feats[img_hard_negatives], 
                                        margin) + self.triplet_margin_loss(image_feats, 
                                        text_feats, 
                                        text_feats[txt_hard_negatives], 
                                        margin)
        return loss
    def get_gauss_construct_loss(self, mean, alpha1, beta, target):
        alpha_eps = 1e-4
        beta_eps = 1e-4
        resi_min = 1e-4
        resi_max = 1e3
        alpha1 = alpha1 + alpha_eps
        beta1 = beta + beta_eps
        resi = torch.abs(mean - target)
        resi = (resi*alpha1*beta1).clamp(min=resi_min, max=resi_max)
        ## check if resi has nans
        if torch.sum(resi != resi) > 0:
            print('resi has nans!!')
            return None
        log_one_over_alpha = torch.log(alpha1)
        ...
        return l.mean()
    def get_prob_loss(self, mean, alpha1, beta, target, t1, t2):
        l1 = F.l1_loss(mean, target, reduction='mean')
        lgau = self.get_gauss_construct_loss(mean, alpha1, beta, target)
        l = t1 * l1 + t2 * lgau
        return l
    
    def get_lse_loss(self, inputs, target):
        ce_loss = torch.nn.functional.cross_entropy(inputs, target, reduction='none')

        # 计算 LogSumExp Loss
        max_ce_loss = ce_loss.max(dim=0).values
        log_sum_exp_loss = max_ce_loss + torch.log(torch.sum(torch.exp(ce_loss - max_ce_loss)))

        return log_sum_exp_loss.mean()

    def get_slot_loss(self, sim1, sim2, margin=0.1):
        sim1 = sim1 / sim1.norm(dim=-1)
        sim2 = sim2 / sim2.norm(dim=-1)
        label = torch.arange(0, sim1.shape[0], dtype=torch.long, device=sim1.device)
        # l1 = torch.clamp(sim1 - sim2 + margin, min=0.0)
        # l1 = torch.max(l1).sum(dim=-1)
        l1 = F.cross_entropy(sim1, label)
        # l2 = torch.clamp(sim2 - sim1 + margin, min=0.0)
        l2 = F.cross_entropy(sim2, label)
        return l1 + l2


    def get_slot_triplet_loss(self, sim1, sim2, margin = 0.001):
        l1 = torch.clamp(sim1 - sim2 + margin, min=0.0)
        l2 = torch.clamp(sim1 - sim2 + margin, min=0.0)
        l1 = torch.max(l1).sum(dim=-1)
        l2 = torch.max(l2).sum(dim=-1)
        return l1 + l2
