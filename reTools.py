...
def evaluation(model, data_loader, tokenizer, device, config, args):
    ...
    for i in range(0, num_text, text_bs):
        ...
        gpt = gpts[i: min(num_text, i + text_bs)]
        ...
        if config['diffusion']:
            t = torch.randint(0, 1000, (text_bs,), device=text_embed.device, dtype=torch.long)
            rand_noise = torch.randn_like(text_embed.unsqueeze(1), device=text_embed.device)
            text_embed = model.ddpm_sampler.p_sample(rand_noise, rand_noise.new_full((1,), 999, dtype=torch.long), text_embed.unsqueeze(1)).squeeze(1)
        text_feat = model.text_proj(text_embed[:, 0, :])
        text_feat = F.normalize(text_feat, dim=-1)
        if config['gmm']:
            text_feat, _, _ = model.text_prob_con(text_feat)
    ...
    clean_noise_sim = gpt_feats @ text_feats.t()
    clean_noise_sim = clean_noise_sim.diagonal().view(-1)
    bar = 0.5
    print(torch.nonzero(clean_noise_sim < bar).squeeze(1))

    text_atts = text_atts[torch.nonzero(clean_noise_sim < bar).squeeze(1)]
    text_feats = text_feats[torch.nonzero(clean_noise_sim < bar).squeeze(1)]
    text_embeds = text_embeds[torch.nonzero(clean_noise_sim < bar).squeeze(1)]
    ...
    for image, img_id in data_loader:
        ...
        if config['gmm']:
            image_feat, _, _ = model.image_prob_con(image_feat)
    ...
    score_matrix_t2i = torch.full((text_atts.shape[0], len(data_loader.dataset.image)), -1000.0).to(device)
    if config['slot']:
        slot_sim = calculate_slot_sim(model, config['batch_size_train'], 
                                      image_embeds, text_embeds, image_feats, text_feats)
    sims_matrix = sims_matrix + slot_sim
    num_tasks = utils.get_world_size()
    rank = utils.get_rank()
    ...
    return score_matrix_t2i.cpu().numpy(), torch.nonzero(clean_noise_sim < bar).squeeze(1)


def mAP(scores_t2i, g_pids, q_pids, index,table=None):
    similarity = torch.tensor(scores_t2i)
    print(similarity.shape)
    indices = torch.argsort(similarity, dim=1, descending=True)
    g_pids = torch.tensor(g_pids)
    q_pids = torch.tensor(q_pids)[index]
    ...
    num_matches_5 = num_matches(matches)
    avg_matches_5 = num_matches_5.float().sum() / q_pids.shape[0]
    avg_matches_5 = avg_matches_5 / 5.0
    if not table:
        table = PrettyTable(["task", "R1", "R5", "R10", "mAP", "mINP", "avg_matches_5"])
        table.add_row(['t2i', t2i_cmc[0], t2i_cmc[4], t2i_cmc[9], t2i_mAP, t2i_mINP, avg_matches_5])
...
def reorganize(input, batch_size):
    b = input.split(dim=0, split_size=batch_size)
    b = list(b)
    temp = b[-2][b[-1].shape[0]:]
    b[-1] = torch.cat([temp, b[-1]], dim=0)
    return b

def calculate_slot_sim(model, batch_size, image_embeds, text_embeds, image_feats, text_feats):
    img_num = image_embeds.shape[0]
    txt_num = text_embeds.shape[0]
    image_embeds = reorganize(image_embeds, batch_size)
    text_embeds = reorganize(text_embeds, batch_size)
    image_feats = reorganize(image_feats, batch_size)
    text_feats = reorganize(text_feats, batch_size)
    sims_t2i = []
    for text_embed, text_feat in zip(text_embeds, text_feats):
        temp = []
        for image_embed, image_feat in zip(image_embeds, image_feats):
            sim, _ = model.agm(image_embed, text_embed, image_feat, text_feat)
            temp.append(sim)
        temp = torch.cat(temp, dim=1)
        sims_t2i.append(temp)
    sims_t2i = torch.cat(sims_t2i, dim=0)

    residual = txt_num % batch_size
    sims_t2i = torch.cat([sims_t2i[:-batch_size], sims_t2i[-residual:]], dim=0)

    residual = img_num % batch_size
    sims_t2i = torch.cat([sims_t2i[:,:-batch_size], sims_t2i[:,-residual:]], dim=1)
    sims_t2i = F.layer_norm(sims_t2i, [sims_t2i.shape[-1]])
    return sims_t2i

def new_calculate_slot_sim(model, image_embeds, text_embeds, image_feats, text_feats):
    with torch.no_grad():
        sims, _ = model.agm(image_embeds, text_embeds, image_feats, text_feats)
    return sims

def num_matches(matches):
    num_matches_5 = matches[:, :5].sum(1)
    return num_matches_5